...
module.exports = {
    addDelivery: async (req, res) => {
        const { receiver, phonenumber, pickup, dropoff, sendorId, size, type, parcel, notes, quantity, deliveryTime, deliveryDate, dropOffCost, pickUpCost, deliveryCost } = req.body;
        ...
        try {
            ...
            await User.updateOne({ _id: sendorId }, {
                $push: {
                    deliveries: {
            ...
            await User.updateOne({ _id: handler.success && handler.body.handler ? handler.body.handler : '6481003e050a57815f7be8f0' }, {
                $push: {
                    deliveries: {
                        $each: [`D00${numCurrentDeliveries + 1}`]
                    }
    ...
    updateDelivery: async (req, res) => {
        ...
        try {
            await db.deliveries.updateOne({ deliveryId }, {
                $set: {
                    ...req.body
                }
            });
    ...
    encryptDeliveryDetails: async (req, res) => {
        ...
        try {
            ...
            deliveryIds.forEach(async (deliveryId, index) => {
                const { vendorId, pickup, dropoff, receiver, notes, phonenumber, currentHandler } = await db.deliveries.findOne({ deliveryId });
                const { fullname, phone, email } = await db.users.findOne({ userId: vendorId });
                deliveryDetail.push({
                    from: {
                        fullname,
                        phone,
    ...
    getPartnerDeliveryHistory: async (req, res) => {
        ...
        try {
            ...
            deliveryIds.forEach(async (deliveryId) => {
                const { orderId, pickup, dropoff, deliveryTime, status, vendorId } = await db.deliveries.findOne({ deliveryId });
                const { name, parcel, quantity, size } = await db.orders.findOne({ orderId });
                const { fullname, avatar } = await db.users.findOne({ userId: vendorId });
                deliveryList.push({
                    delivery: {
                        pickup,
                        dropoff,
